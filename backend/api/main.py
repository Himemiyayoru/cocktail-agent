...
try:
    ...
    print(f"[SYSTEM] Loaded {len(COCKTAILS_DB)} cocktails into memory.")
...
def match_cocktails(req: MatchRequest):
    ...
    for idx, c in enumerate(COCKTAILS_DB):
        ...
        missing_original = [comp["ingredient_name"] for comp in recipe_comps if comp["ingredient_name"].lower() in missing]
...
def find_substitute(ingredient: str, limit: int = 3):
    ...
    for name, data in INGREDIENTS_DICT.items():
        ...
        diff_ph = abs(target["physics"]["ph"] - data["physics"]["ph"]) * 5
...
# ==========================================
# 5. Hardcore Physics Simulation Engine
# ==========================================
...
async def chat_with_bob(req: BobChatRequest):
